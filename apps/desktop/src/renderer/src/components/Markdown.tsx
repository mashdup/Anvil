...
// enough to walk the block's raw text (the rendered <code > may be
...
const MD_COMPONENTS_BASE = { code: MdCode, pre: MdPre }
...
export function Markdown({ text, onOpenUrl }: { text: string; onOpenUrl?: (url: string) => void }): React.JSX.Element {
  const components = useMemo(() => {
    if (!onOpenUrl) return MD_COMPONENTS_BASE
    return {
      ...MD_COMPONENTS_BASE,
      a: ({ href, children, node }: { href?: string; children?: React.ReactNode; node?: unknown }) => (
        <MdLink href={href} node={node} onOpenUrl={onOpenUrl}>
          {children}
        </MdLink>
      ),
    }
